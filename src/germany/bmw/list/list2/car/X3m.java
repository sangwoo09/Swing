...
import germany.bmw.list.list2.car.detailRepo.X3mRepo;

public class X3m extends JFrame{
...
JButton b1 = new JButton("リストに戻る");
	...
	private void Center() {
		...
		X3mRepo xRepo = new X3mRepo();
		String[] x3m = xRepo.detail();
		
		JTextField t1 = new JTextField(x3m[1]);
		JTextField t2 = new JTextField(x3m[2] + "円");
		JTextField t3 = new JTextField(x3m[3]);
		JTextField t4 = new JTextField(x3m[4]);
		JTextField t5 = new JTextField(x3m[5]);
		JTextField t6 = new JTextField(x3m[6]);
		JTextField t7 = new JTextField(x3m[7]);
		JTextField t8 = new JTextField(x3m[8]);
